...
import numpy as np
from model.yy_encoder import YyEncoder
...
class GRANMixtureBernoulli(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.yy_encoder   = YyEncoder(self.context_dim)
    self.context_mlp  = nn.Linear(self.context_dim, self.hidden_dim)
  ...
  def _inference(self,
    ...
    if context_vec is not None:
        inj = self.context_mlp(context_vec)
        inj = inj.unsqueeze(1).repeat(1, C*N_max, 1).reshape(-1, H)
        node_feat[:inj.size(0)] += inj       
  ...
  def _sampling(self, B, context_vec=None):
    """ generate adj in row-wise auto-regressive fashion """
    with torch.no_grad():
        ...
        if context_vec is not None:
            inj = self.context_mlp(context_vec)          # B × H
            node_state[:, 0, :H] += inj                  # first node only
        ...
        for ii in range(0, N_pad, S):
          ...
          if context_vec is not None:
                node_state_in[:, ii:ii+K, :H] += inj.unsqueeze(1)
  ...
  def forward(self, input_dict, context_vec=None):
    """
      B: batch size
    ...
    if not is_sampling:
      ...
      A = self._sampling(batch_size, context_vec=context_vec)

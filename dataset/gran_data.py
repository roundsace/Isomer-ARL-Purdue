...
class GRANData(object):
  ...
  def _get_graph_data(self, G):
    ...
    adj_0 = np.array(nx.to_numpy_array(G, dtype=float))
    
    ### Degree descent ranking
    # N.B.: largest-degree node may not be unique
    degree_sequence = sorted(
        node_degree_list, key=lambda tt: tt[1], reverse=True)
    adj_1 = np.array(nx.to_numpy_array(G, nodelist=[dd[0] for dd in degree_sequence], dtype=float))


    ### Degree ascent ranking
    degree_sequence = sorted(node_degree_list, key=lambda tt: tt[1])
    adj_2 = np.array(nx.to_numpy_array(G, nodelist=[dd[0] for dd in degree_sequence], dtype=float))
    ...
    adj_3 = np.array(nx.to_numpy_array(G, nodelist=node_list_bfs, dtype=float))
    adj_4 = np.array(nx.to_numpy_array(G, nodelist=node_list_dfs, dtype=float))
    ...
    adj_5 = np.array(nx.to_numpy_array(G, nodelist=node_list, dtype=float))

...
__all__ = [
    ...
    'preprocess_graph_list', 'create_graphs',"scheme_to_graph"
]


def scheme_to_graph(level_scheme_array):
    """Convert one level‑scheme array to a single networkx.Graph.
    * node 0  →  'grnd'
    * Band‑1 levels become indices 1‑4, Band‑2 → 5‑8, Band‑3 → 9‑12, …
    * All edges are indexed accordingly.
    Returns (G, idx_map) where idx_map maps original names ➜ integers.
    """
    G, idx_map, next_idx = nx.Graph(), {"grnd": 0}, 1

    # give every unique level a consecutive index
    for band in level_scheme_array:
        for lvl in band["nodes"]:
            if lvl != "grnd" and lvl not in idx_map:
                idx_map[lvl] = next_idx
                next_idx += 1

    # add nodes with attributes
    for name, idx in idx_map.items():
        energy = 0 if name == "grnd" else next(
            band["nodes"][name] for band in level_scheme_array
            if name in band["nodes"]
        )
        G.add_node(idx, label=name, energy=energy)
...
def create_gamma_graphs(data_dir):
    """
    Walk through *.npz files in `data_dir` and return a list
    [(G, matrix), …] where *G* is built with scheme_to_graph.
    """
    graphs = []
    ...
    for fname in sorted(os.listdir(data_dir)):
        ...
        mats    = [npz[k] for k in sorted(npz.files) if k.startswith("matrix_")]
        schemes = [npz[k] for k in sorted(npz.files) if k.startswith("level_scheme_")]

        for scheme, mat in zip(schemes, mats):
            G, _ = scheme_to_graph(scheme)
            G.graph["source_file"] = fname
            graphs.append((G, mat.astype(np.float32)))
...
def create_graphs(graph_type, data_dir='data', noise=10.0, seed=1234):
  ...
  if graph_type == 'grid':
    ...
    tuple_list = create_gamma_graphs(data_dir)   # [(G, matrix), ...]
    graphs     = [g for g, _ in tuple_list]      # strip matrices
    return graphs
